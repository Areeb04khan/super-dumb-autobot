# -*- coding: utf-8 -*-
"""
Super Dumb Heroes Bot v6.2 (The Bulletproof JSON Update)
- Writing: High-IQ satirical comedy (OpenRouter/Groq) with Regex & Retry Loops
...
# ============================================================
# TIER 1: HIGH-IQ COMEDY GENERATOR (With JSON Regex & Retries)
# ============================================================
...
def generate_satirical_script() -> dict:
    ...
    for provider in fallbacks:
        ...
        # Give each provider up to 3 tries in case they return bad JSON
        for attempt in range(1, 4):
            try:
                print(f"🔄 Querying {provider['name']} (Attempt {attempt}/3)...")
                ...
                # Regex to extract only the JSON dictionary, ignoring extra AI chatter
                json_match = re.search(r'\{.*\}', raw, re.DOTALL)
                if not json_match:
                    print(f"⚠️ No JSON found in response on attempt {attempt}.")
                    continue
                ...
                print(f"⚠️ JSON Decode Error on attempt {attempt}: {e}. Retrying...")
            except Exception as e:
                print(f"⚠️ {provider['name']} connection/API failed: {e}")
                break # If the API itself is down, break and try the next provider

    raise RuntimeError("All LLM providers failed to return valid JSON after multiple attempts.")
